...
public class PaymentDaoImpl extends AbstractCrudDaoImpl<Payment> implements PaymentDao {
    ...
    private static final String CREATE_ORDER_BY_PARAMETERS = "INSERT INTO " +
            "orders(user_id, user_document, cabin_status_id, order_date, order_status_id, cruise_id) " +
            "VALUES (?, ?, ?, ?, ?, ?);";
    ...
    private static final String FIND_PAYMENTS_BY_USER_ID_AND_ORDER_STATUSES = "SELECT * FROM payments " +
            ...
            "LEFT JOIN cruise_statuses cs2 on cs2.cruise_status_id = c.cruise_status_id " +
            "WHERE u.user_id = ? " +
            "AND (o.order_status_id=? OR o.order_status_id=? OR o.order_status_id=? OR o.order_status_id=?) " +
            "ORDER BY ? " +
            "LIMIT ? OFFSET ?;";

    private static final String FIND_PAYMENTS_BY_ORDER_STATUSES = "SELECT * FROM payments " +
            "LEFT JOIN orders o on payments.order_id = o.order_id " +
            "LEFT JOIN users u on u.user_id = o.user_id " +
            "LEFT JOIN cabin_statuses cs on cs.cabin_status_id = o.cabin_status_id " +
            "LEFT JOIN cabins ca on ca.cabin_id = cs.cabin_id " +
            ...
            "LEFT JOIN routes r on r.route_id = c.route_id " +
            "LEFT JOIN cruise_statuses cs2 on cs2.cruise_status_id = c.cruise_status_id " +
            "WHERE o.order_status_id=? OR o.order_status_id=? OR o.order_status_id=? OR o.order_status_id=? " +
            "ORDER BY ? " +
            "LIMIT ? OFFSET ?;";
    ...
    private static final String COUNT_ALL_BY_ORDER_STATUSES = "SELECT count(*) AS count_payments " +
            "FROM payments LEFT JOIN orders o on o.order_id = payments.order_id " +
            "WHERE o.order_status_id=? OR o.cabin_status_id=? OR o.order_status_id=? OR o.order_status_id=?; ";

    private static final String UPDATE_PAYMENT_STATUS_BY_PAYMENT_ID = "UPDATE payments SET payment_status_id = ? " +
            "WHERE payment_id = ?";
    ...
    public Payment addPayment(Payment payment) {
        try (Connection connection = connector.getConnection();
            ...
            try {
                ...
                createOrder.setString(2, payment.getOrder().getUserDocuments());
                createOrder.setInt(3, payment.getOrder().getCabinStatus().getId());
                createOrder.setDate(4, Date.valueOf(DATE_NOW));
                createOrder.setInt(5, ORDER_STATUS_IN_PROGRESS);
                createOrder.setInt(6, payment.getOrder().getCruise().getId());
                createOrder.execute();
    ...
    @Override
    public Integer countAllPaymentsByUserIdAndFilterParameters(PaymentRequestDtoUtil paymentRequestDtoUtil) {
    ...
    @Override
    public Integer countAllPaymentsByFilterParameters(PaymentRequestDtoUtil paymentRequestDtoUtil) {
        Integer[] paymentStatusIds = paymentRequestDtoUtil.getPaymentStatusIds();
        try (Connection connection = connector.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(COUNT_ALL_BY_ORDER_STATUSES)) {
    ...
    @Override
    public List<Payment> findPaymentsByFilterParameters(PaymentRequestDtoUtil paymentRequestDtoUtil, Integer firstRecordOnPage) {
        String sortingAndOrdering = String.format("%s %s", paymentRequestDtoUtil.getSorting(), paymentRequestDtoUtil.getOrdering());
        Integer[] paymentStatusIds = paymentRequestDtoUtil.getPaymentStatusIds();
        try (Connection connection = connector.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(FIND_PAYMENTS_BY_ORDER_STATUSES)) {
            preparedStatement.setInt(1, paymentStatusIds[0]);
            preparedStatement.setInt(2, paymentStatusIds[1]);
            preparedStatement.setInt(3, paymentStatusIds[2]);
            preparedStatement.setInt(4, paymentStatusIds[3]);
            preparedStatement.setString(5, sortingAndOrdering);
            preparedStatement.setInt(6, paymentRequestDtoUtil.getItemsOnPage());
    ...
    @Override
    public void updatePaymentStatusByPaymentId(Integer paymentId, Integer paymentStatusId, Integer orderStatusId) throws SQLException {
        Connection connection = connector.getConnection();
        PreparedStatement updatePayment = connection.prepareStatement(UPDATE_PAYMENT_STATUS_BY_PAYMENT_ID);
        PreparedStatement updateOrder = connection.prepareStatement(UPDATE_ORDER_STATUS_BY_PAYMENT_ID);
        try {
            connection.setAutoCommit(false);
            updatePayment.setInt(1, paymentStatusId);
            updatePayment.setInt(2, paymentId);
            updatePayment.executeUpdate();

            updateOrder.setInt(1, orderStatusId);
            updateOrder.setInt(2, paymentId);
            updateOrder.executeUpdate();
            connection.commit();
            LOGGER.info("The transaction for updatePayment completed");
        } catch (SQLException e) {
            connection.rollback();
            LOGGER.info("Rollback of updatePayment transaction");
            throw new DataBaseRuntimeException(e);
        } finally {
            connection.close();
        }
    ...
    @Override
    public List<Payment> findPaymentsByUserIdAndFilterParameters(PaymentRequestDtoUtil paymentRequestDtoUtil, Integer firstRecordOnPage) {
        String sortingAndOrdering = String.format("%s %s", paymentRequestDtoUtil.getSorting(), paymentRequestDtoUtil.getOrdering());
        Integer[] paymentStatusIds = paymentRequestDtoUtil.getPaymentStatusIds();
        try (Connection connection = connector.getConnection();
